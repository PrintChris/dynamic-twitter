...
for (let i = 0; i < users.length; i++) {
    tweetCounter.push({
        ...
        tweetCount: users[i].tweets.length
    })
};
...
const appendSearchStringToURL = () => {
    ...
    selectUserId.addEventListener("change", (event) => {
        window.location.href = window.location.origin + '?user=' + event.target.value;
    });
};
...
if (activeUserName === null) {
    ...
    if (window.location.pathname === '/timeline.html') {
        ...
        <h3>Select a user ...</h3>
        `
        appendToDropdownUserSelector('body');
        appendSearchStringToURL();
    }
    ...
    if (userSelector.verified) {
        verifiedTick = ` <img src='assets/verified.png'>`;
    };
    ...
    // Following
    const followingCountSelector = compactFollowCount(userSelector.followingCount);
    ...
    function setNewActive(el) {
        ...
        document.getElementById(el.replace("tab", "tab-text")).classList.add("tab-text-active");
        ...
        document.getElementById(el.replace("tab", "body")).classList.add("content-active");
    }
...
function generateTweetsForDisplay() {
    ...
    */
    ...
    tweetsForDisplay.forEach(tweet => {
        if (tweet.verified) {
            tweet.verifiedTick = ` <img src='assets/verified.png'>`;
        } else {
            tweet.verifiedTick = "";
        }
    });
...
function filterTweetsForDisplay(user) {
    return tweetSelector = tweetsForDisplay.filter(property => property.userName === user);
}
